...
const DeliveryIssueDetails = ({ data }) => {
  ...
  return (
    <div className="border border-green-200 bg-green-50 rounded-2xl p-6 space-y-6">
      ...
      {saved_address && (
        <AddressCard title="Saved Address" address={saved_address} />
      )}
          ...
          {products?.map((item, index) => (
            <div key={index} className="border rounded-lg p-4">
              ...
              <div className="grid md:grid-cols-3 gap-4 mt-3">
                <Info
                  label="Status"
                  value={item.order_item?.status}
                />
                <Info label="Quantity" value={item.order_item?.quantity} />
                <Info label="Size" value={item.order_item?.size} />
                <Info
                  label="Return Requested"
                  value={item.order_item?.return_requested ? "Yes" : "No"}
                />
                <Info
                  label="Cancelled"
                  value={item.order_item?.is_cancelled ? "Yes" : "No"}
                />
                <Info
...
const AddressCard = ({ title, address }) => (
  <Section title={title}>
    ...
    <Info label="Email" value={address?.email} />
    <Info label="Mobile" value={address?.mobile} />
    <Info label="Street" value={address?.street_address} />
